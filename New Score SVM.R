...
clean_normalize <- function(drug_string){
  ...
  normalized_mean<-sum(X_test_final)/sd(trainRowSums)
...
summary(trainPositive)

trainPositive_1=trainPositive[rowSums(trainPositive[,NaturalPredictors]) > quantile(rowSums(trainPositive[,NaturalPredictors]), 0.1),]

 
inTrain<-createDataPartition(1:nrow(trainPositive_1),p=0.8,list=FALSE)



trainpredictors<-trainPositive_1[inTrain,NaturalPredictors]
trainLabels<-trainPositive_1[inTrain,"y"]


table(trainLabels)

trainRowSums=rowSums(trainpredictors)
trainTarget=trainLabels
...
#Document Term Matrix, because we need to be organized folk
cleandrug_test_dtm <- DocumentTermMatrix(cleandrugs_test_corp)
...
minmaxtest<-(trainRowSums-min(trainRowSums))/(max(trainRowSums)-min(trainRowSums))
summary(trainRowSums)
summary(rowSums(testpredictors))
trainRowSums<-c(2,1,trainRowSums)

trainTarget<-c(TRUE,TRUE,trainTarget)
...
minmaxtesttest<-(rowSums(testpredictors)-min(trainRowSums))/(max(trainRowSums)-min(trainRowSums))
