...
clean_normalize <- function(drug_string){
  ...
  return(min(sum(X_test_final), max(trainRowSums)));
}
...
save(AllPredictors, file="AllPredictors.RData")
...
AllYouNeed<-c(mean(trainRowSums),sd(trainRowSums), min(trainRowSums), max(trainRowSums))

names(AllYouNeed)<-c("Mean", "SD", "MIN", "MAX")

save(AllYouNeed, file="AllYouNeed.RData")
